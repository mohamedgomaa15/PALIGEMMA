...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        # [B, C, H, W]  -> [B, embedd, num_patch_H, num_patch_W]
        patch_embed = self.patch_embedding(pixel_values)
         # [B, embedd, num_patch_H, num_patch_W] -> [B, embedd,  num_patches]
        embeddings = patch_embed.flatten(2)
        #  [B, embedd,  num_patches]  -> [B, num_patches,  embed] 
...
class SiglipAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.hidden_size % config.num_attention_heads != 0:
            raise ValueError("Hidden Size cannot be divises int heads")
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        value_states = self.v_states(hidden_states)
        ...
        if attn_weights.size() != (batch_size, self.config.num_attention_heads, seq_len, seq_len):
            raise ValueError(f"Attention weight should be of size {(batch_size, self.config.num_attention_heads, seq_len, seq_len)}")
        # [B, num_heads, num_patches, num_patches]
        ...
        if attn_output.size() != (batch_size, self.config.num_attention_heads, seq_len, self.head_dim):
            raise ValueError(f"Attention output should be of size {(batch_size, self.config.num_attention_heads, seq_len, self.head_dim)}")
...
class SiglipMLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = nn.functional.gelu(hidden_states, approximate="tanch")
...
class  SiglipVisionTransformer(nn.Module):
    ...
    def forward(self, pixel_values) -> torch.tensor:
        ...
        return self.post_layernorm(last_hidden_states)
